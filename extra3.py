...
if(argc!=3 and argc!=2):
    print 'Usage pyton %s word number' %argv[0]
    quit()
if(argc==3):
	if(int(sys.argv[2])>47 or int(sys.argv[2])==0):
		print 'Usage pyton %s word 1~47' %argv[0]
		quit()
	else:
		countv=0
		for i in argv[2]:
		...
		if(countv==2):
			kari=[]
			kari='PREF'+sys.argv[2]
		if(countv==1):
			kari=[]
			kari='PREF0'+sys.argv[2]
####
# APIアクセス
####
# URLに続けて入れるパラメータを組立
